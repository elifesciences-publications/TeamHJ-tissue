...
derivs(Tissue &T,
  ...
  for (size_t cellIndex=0 ; cellIndex<numCells ; ++cellIndex) {
    ...
    // if(cellData[cellIndex][29]==1 && cellData[cellIndex][28]==-1){ //L1 bottom
    //   youngL = sEpi*youngL;
    //   youngT = sEpi*youngT; 
    // }
...
update(Tissue &T,
  ...
  for (size_t cellIndex=0 ; cellIndex<numCells ; ++cellIndex) {
    ...
    if(neighborweight==0){
      ...
      double missesStress=std::sqrt(maximalStressValue  * maximalStressValue  +
                                    maximalStressValue2 * maximalStressValue2);
      cellData[cellIndex][MTstressIndex ]=missesStress;
      // storing a measure for stress anisotropy in cell vector
      if (std::fabs(maximalStressValue)<  0.000001) cellData[cellIndex][stressAnIndex]=0;
    ...
    //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      ...
      cellData[cellIndex][anisoEnergyIndex]=TotalCellRestingArea;//EnergyAniso/TotalCellRestingArea;
