...
derivs(Tissue &T,
  ...
  for( size_t cellIndex=0 ; cellIndex<numCells ; ++cellIndex ) {
    ...
    //Current shape local coordinate of the element  (counterclockwise ordering of nodes/edges)
      ...
      // // cellData[cellIndex][variableIndex(0,3)] :  total stress aniso
      // else  sigmafactor=0;
...
derivs(Tissue &T,
  ...
  for (size_t cellIndex=0 ; cellIndex<numCells ; ++cellIndex) {
    ...
    if (std::abs(maximalStrainValue) >= 0.000001) cellData[cellIndex][strainAnIndex]=1-std::abs(maximalStrainValue2/maximalStrainValue);
